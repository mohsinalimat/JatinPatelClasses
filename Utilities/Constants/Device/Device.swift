...
open class Device {
    ...
    /// Use this method to get the size of the current device.
    ///
    /// ````
    /// Device.size()
    /// ````
    /// - Returns: Size enum. for e.g - .screen3_5Inch | .screen4Inch | .screen4_7Inch | .screen5_5Inch | .screen5_8Inch
    ...
    /// Use this method to get the type of the Device
    ///
    /// ````
    /// Device.type()
    /// ````
    /// - Returns: Type enum for the device. for e.g - .iPhone | .iPad | .iPod | .simulator |
    ...
    /// Check that if the display is Retina display or not.
    ///
    /// ````
    /// Device.isRetina()
    /// ````
    /// - Returns: **true** if display is Retina display
    ...
    /// Device.isPod()
    /// ````
    /// - Returns: **true** if device is iPod
    static open func isPod() -> Bool {
        return type() == .iPod
    }
    
    /// Check if the device is Similator?
    ///
    /// ````
    /// Device.isSimulator()
    /// ````
